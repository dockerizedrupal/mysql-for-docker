...
if [ "${1}" = "backup" ]; then
  ...
  if [ -n "${CONTAINERS}" ]; then
    for CONTAINER in ${CONTAINERS}; do
      ...
      docker run \
        ...
        --entrypoint /bin/bash \
        dockerizedrupal/mysql:1.2.0 -c "tar czvf /backup/${CONTAINER_NAME}.tar.gz /mysql"
    done
  fi
elif [ "${1}" = "restore" ]; then
  for FILE in *.tar.gz; do
    ...
    docker run \
      ...
      -v /mysql \
      --entrypoint /bin/echo \
      dockerizedrupal/mysql:1.2.0 "Data-only container for MySQL."
    ...
    docker run \
      ...
      -v "${WORKING_DIR}:/backup" \
      --entrypoint /bin/bash \
      dockerizedrupal/mysql:1.2.0 -c "tar xzvf /backup/${CONTAINER}.tar.gz"
  done
else
  unknown_command
fi
